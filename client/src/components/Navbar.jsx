...
const Navbar = ({ userInfo, onSearchNote, handleClearSearch, variant = "default" }) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const dropdownRef = useRef(null);
  const navigate = useNavigate();
  ...
  return (
    <div className={`${bgClass} flex items-center justify-between px-4 py-4 shadow-custom-bottom-left md:px-6 md:py-4 sm:rounded-xl`}>
      <h2 className='text-2xl font-medium text-black py-2'>
        Keeper Notes
      </h2>
      ...
      {userInfo && (
        <div className='relative ml-4' ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div className={`absolute right-0 mt-2 w-36 py-2 bg-white border rounded-lg shadow-lg transition-transform duration-300 transform ${dropdownOpen ? 'scale-100' : 'scale-0'}`}>
              <p className='px-4 py-4 text-sm font-medium text-black'>{userInfo.fullName}</p>
              <button 
                className='w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100'
                onClick={() => {
